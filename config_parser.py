...
class ConfigParser(ConfigObj):
    ...
    def __init__(self, config_file, **kwargs):
        ConfigObj.__init__(self, config_file, **kwargs)
    ...
    def is_valid(self):
        if set(self.motioncor_options).issubset(self.motioncor_parameters | {'path_to_executable', 'timeout', 'trials'}):
            self.logger.info("Motioncor options are valid")
        else:
            self.logger.error("Motioncor options are not valid")
            return False
        if set(self.gctf_options).issubset(self.gctf_parameters | {'path_to_executable', 'timeout', 'trials', 'cc_cutoff'}):
            self.logger.info("Gctf options are valid")
        else:
            self.logger.error("Gctf options are not valid")
            return False
        return True